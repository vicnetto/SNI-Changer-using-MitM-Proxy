...
int load_root_ca_key_and_crt(const char *root_ca_key_path,
  ...
  *root_ca_key = PEM_read_bio_PrivateKey(bio, NULL, NULL, NULL);
  if (!root_ca_key)
    goto error;
...
int generate_certificate_key(EVP_PKEY **key, X509_REQ **req) {
  ...
  *req = X509_REQ_new();
  ...
  X509_NAME_add_entry_by_txt(name, "O", MBSTRING_ASC,
                             (const unsigned char *)DN_ORGANIZATION, -1, -1, 0);
  X509_NAME_add_entry_by_txt(name, "OU", MBSTRING_ASC,
                             (const unsigned char *)DN_ORGANIZATION_UNIT, -1,
                             -1, 0);
  X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC,
                             (const unsigned char *)DN_COMMON_NAME, -1, -1, 0);
...
int sign_certificate(EVP_PKEY *ca_key, X509 *ca_crt, EVP_PKEY **key,
                     X509 **crt) {
  ...
  *crt = X509_new();
  ...
  char *san_dns = "www.google.com";
  GENERAL_NAMES *gens = sk_GENERAL_NAME_new_null();
  GENERAL_NAME *gen = GENERAL_NAME_new();
  ASN1_IA5STRING *ia5 = ASN1_IA5STRING_new();
  ASN1_STRING_set(ia5, san_dns, strlen(san_dns));
...
int generate_certificate(char *root_ca_key_path, char *root_ca_crt_path,
  ...
  int ret = sign_certificate(root_ca_key, root_ca_crt, key, crt);
  if (!ret) {
